...
export const ProductType = {
  makeSerializer(
    ...
    // `fieldNames`/`fieldSerializers` are populated *after* the closure is
    // cached, so that recursive types resolve through the cache instead of
    // recursing forever.
  ...
  makeDeserializer(
    ...
    // `fieldNames`/`fieldDeserializers` are populated *after* the closure is
    // cached, so that recursive types resolve through the cache instead of
    // recursing forever.
    const fieldNames: string[] = [];
...
export const SumType = {
  makeSerializer(ty: SumTypeType, typespace?: TypespaceType): Serializer<any> {
    if (
      ...
      // Prototype-less so that variant names like `toString` or `__proto__`
      // can't resolve to inherited members. Populated *after* the closure is
      // cached, so recursive types resolve through the cache.
  ...
  makeDeserializer(
    ...
    if (
      ...
      let deserializer = DESERIALIZERS.get(ty);
      if (deserializer != null) return deserializer;
      // Populated *after* the closure is cached, so recursive types resolve
      // through the cache.
      ...
      deserializer = reader => {
        const tag = reader.readU8();
        // The previous `switch`-based implementation had no `default`, so
        // out-of-range tags returned `undefined`; preserved here.
        if (tag >= numVariants) return undefined;
        return {
